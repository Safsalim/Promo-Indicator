...
# Print initial structure for debugging
print(f"S&P 500 shape: {sp500.shape}")
print(f"Columns: {sp500.columns}")

# Convert to simple format using the index and Close price
sp500_data = pd.DataFrame({
    'date': sp500.index.date,
    'sp500_close': sp500[('Close', '^GSPC')].values,
    'sp500_open': sp500[('Open', '^GSPC')].values,
    'sp500_high': sp500[('High', '^GSPC')].values,
    'sp500_low': sp500[('Low', '^GSPC')].values,
    'sp500_volume': sp500[('Volume', '^GSPC')].values
})
...
sp500_subset = sp500_data[['date', 'sp500_close']].copy()
...
print(f"Columns in combined data: {df_combined.columns.tolist()}")

# Use the correct date column
date_column = 'date_x' if 'date_x' in df_combined.columns else 'date'
...
# Plot 1: Views vs BTC
ax1_twin = ax1.twinx()
line1 = ax1.plot(df_combined[date_column], df_combined['views_7day_ma'], 'b-', linewidth=2, label='7-Day MA Views')
line2 = ax1_twin.plot(df_combined[date_column], df_combined['btc_price_close'], 'orange', linewidth=2, label='BTC Price')
...
ax2_twin = ax2.twinx()
line3 = ax2.plot(df_combined[date_column], df_combined['views_7day_ma'], 'b-', linewidth=2, label='7-Day MA Views')
line4 = ax2_twin.plot(df_combined[date_column], df_combined['sp500_close'], 'green', linewidth=2, label='S&P 500 Price')
...
ax3_twin = ax3.twinx()
line5 = ax3.plot(df_combined[date_column], df_combined['btc_price_close'], 'orange', linewidth=2, label='BTC Price')
line6 = ax3_twin.plot(df_combined[date_column], df_combined['sp500_close'], 'green', linewidth=2, label='S&P 500 Price')
...
print(f"Data period: {df_combined[date_column].min().strftime('%Y-%m-%d')} to {df_combined[date_column].max().strftime('%Y-%m-%d')}")
...
for start, end, label in quarters:
    mask = (df_combined[date_column] >= start) & (df_combined[date_column] <= end)
...
print(f"3. VOLATILITY COMPARISON: BTC is {btc_volatility/sp500_volatility:.1f}x more volatile than S&P 500")
...
# Market vs views performance
print(f"\n=== PERFORMANCE COMPARISON ===")
print(f"Average views during high BTC periods (>${df_combined['btc_price_close'].quantile(0.75):.0f}): {df_combined[df_combined['btc_price_close'] > df_combined['btc_price_close'].quantile(0.75)]['views_7day_ma'].mean():.0f}")
print(f"Average views during low BTC periods (<${df_combined['btc_price_close'].quantile(0.25):.0f}): {df_combined[df_combined['btc_price_close'] < df_combined['btc_price_close'].quantile(0.25)]['views_7day_ma'].mean():.0f}")
print(f"Average views during high S&P periods (>${df_combined['sp500_close'].quantile(0.75):.0f}): {df_combined[df_combined['sp500_close'] > df_combined['sp500_close'].quantile(0.75)]['views_7day_ma'].mean():.0f}")
print(f"Average views during low S&P periods (<${df_combined['sp500_close'].quantile(0.25):.0f}): {df_combined[df_combined['sp500_close'] < df_combined['sp500_close'].quantile(0.25)]['views_7day_ma'].mean():.0f}")

# Summary table
print(f"\n=== SUMMARY TABLE ===")
summary_stats = pd.DataFrame({
    'Metric': ['Correlation with Views', 'Annualized Volatility (%)', 'Total Return (%)', 'Avg Views (High Period)', 'Avg Views (Low Period)'],
    'BTC': [
        f"{btc_correlation:.3f}",
        f"{btc_volatility:.1f}",
        f"{btc_return:.1f}",
        f"{df_combined[df_combined['btc_price_close'] > df_combined['btc_price_close'].quantile(0.75)]['views_7day_ma'].mean():.0f}",
        f"{df_combined[df_combined['btc_price_close'] < df_combined['btc_price_close'].quantile(0.25)]['views_7day_ma'].mean():.0f}"
    ],
    'S&P 500': [
        f"{sp500_correlation:.3f}",
        f"{sp500_volatility:.1f}",
        f"{sp500_return:.1f}",
        f"{df_combined[df_combined['sp500_close'] > df_combined['sp500_close'].quantile(0.75)]['views_7day_ma'].mean():.0f}",
        f"{df_combined[df_combined['sp500_close'] < df_combined['sp500_close'].quantile(0.25)]['views_7day_ma'].mean():.0f}"
    ]
})
