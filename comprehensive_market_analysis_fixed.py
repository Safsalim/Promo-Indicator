...
print(f"Initial S&P 500 shape: {sp500.shape}")
print(f"Initial columns: {sp500.columns.tolist()}")

# Flatten the multi-level columns if they exist
if isinstance(sp500.columns, pd.MultiIndex):
    # Use the top level for price columns and date for the index
    sp500.columns = [col[1] for col in sp500.columns]
    
# Reset index to make Date a column
sp500.reset_index(inplace=True)

# Print final structure
print(f"Final S&P 500 shape: {sp500.shape}")
print(f"Final columns: {sp500.columns.tolist()}")
...
sp500_subset = sp500[['date', 'sp500_close']].copy()
