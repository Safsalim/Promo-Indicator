...
class LiveStreamMetrics {
  ...
  static excludeById(id, reason = 'manual', metadata = null) {
    const db = getDatabase();
    const stmt = db.prepare(`
      UPDATE live_stream_metrics 
      SET is_excluded = 1,
      ...
      WHERE id = ?
    `);
    return stmt.run(reason, metadata ? JSON.stringify(metadata) : null, id);
  }

  static excludeDay(channelId, date, reason = 'manual', metadata = null) {
    const db = getDatabase();
    const stmt = db.prepare(`
      UPDATE live_stream_metrics 
      SET is_excluded = 1,
      ...
      WHERE channel_id = ? AND date = ?
    `);
    return stmt.run(reason, metadata ? JSON.stringify(metadata) : null, channelId, date);
  }
  ...
  static findAutoExcluded() {
    const db = getDatabase();
    const stmt = db.prepare(`
      SELECT lsm.*, c.channel_handle, c.channel_name
      FROM live_stream_metrics lsm
      JOIN channels c ON lsm.channel_id = c.id
      WHERE lsm.is_excluded = 1 AND lsm.exclusion_reason = 'auto_anomaly_detection'
      ORDER BY lsm.excluded_at DESC
    `);
    return stmt.all();
  }
